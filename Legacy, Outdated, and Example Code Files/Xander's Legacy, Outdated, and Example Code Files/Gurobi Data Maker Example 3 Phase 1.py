...
x = 2000 # Solar Capacity
y = 1 # Fuel cost

# Define the settings to tweak
tweak_settings = [
    "BQPCuts",
    "CliqueCuts",
    "CoverCuts",
...
# Lower fuel cost
gen_df['fuel_cost'] = y*gen_df['cost_per_mmbtu'].fillna(0)
...
for i in range(-1, 4):  # Cuts range from -1 to 3
    print(i, None, None)
    ...
    gen_df_sens.loc[gen_df_sens['resource'] == "solar_photovoltaic", 'existing_cap_mw'] = x
    ...
    results.append([i, "None", "N/A", mip_gap_value])

    #--------------------------------------------------------------------------------------------------

    for setting in tweak_settings:
        for j in range(0,3):
            print(i,setting,j)
            ...
            # Add the result to the results list
            results.append([i, setting, j, mip_gap_value])

# Convert the results to a DataFrame
results_df = pd.DataFrame(results, columns=["Cuts", "Setting", "Value", "Gap"])

# Write the results to a CSV file
results_df.to_csv("solver_tuning_results_example3_phase1.csv", index=False)
